...
# ---------- GENERATE ----------
if generate_btn:
    if not subject or not topic or not grade or not duration:
        st.warning("Fill all fields before generating.")
    else:
        prompt_text = f"""
...
"""
        try:
            ...
            st.text_area("Generated Lesson", value=lesson_text, height=300)
...
# ---------- ADD ----------
if add_btn:
    if not subject or not topic or not grade or not duration or not lesson_text:
        st.warning("Fill all fields including lesson text.")
    else:
